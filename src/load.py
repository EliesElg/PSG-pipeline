...
def from_s3_to_bq(date_dag=None):
    if not date_dag:
        today = datetime.today().date()
        date_dag = today.strftime("%Y-%m-%d")
    ...
    try:
        response = S3_client.get_object(
            Bucket=os.getenv("BUCKET_NAME"), Key=f"psg_matches_{date_dag}.json"
        )
    ...
    schema = [
        ...
        bigquery.SchemaField(
            "raw_content", "JSON", mode="NULLABLE"
        ),  # "JSON" est le type magique
    ]
